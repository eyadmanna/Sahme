...
<script>
    $(document).on("change", "select.location_province", function () {
        ...
        {{--var blockUI = new KTBlockUI(cities_block, {--}}
        {{--    message: '<div class="blockui-message"><span class="spinner-border text-primary"></span> @lang("lang.Please wait")...</div>',--}}
        {{--});--}}

        if (province_id !== '') {
            // blockUI.block();
            this_city.empty().trigger('change');
    ...
    $(document).on("change", "select.location_city", function () {
        ...
        if (city_id !== '') {
            // blockUI.block();
            this_area.empty().trigger('change');
